...
\\
For reaching the goal in this game, a way for finding the shortest path to exit the maze has to be found. This can be achieved with classical pathfinding algorithms like Dijkstras Algorithm or Breadth-First Search, but also with \textbf{Dynamic Programming}, which I will be using in this Paper.
\\
Therefore, I define a grid map $M=\mathbb{N}^(nxm)$ containing of static walls $M(i,j)=\infty$ and exit(s) of the labyrinth.
...
\subsubsection{Which characteristics needs a problem in compter science to be solvable by Dynamic Programming}
\paragraph{Overlapping subproblems}
\paragraph{Optimal substructure}
\subsubsection{How to apply Dynamic Programming}
...
\section{Conclusion}
The conclusion goes here. Lollo
